...
export class WarpEffect {
  ...
  private targetMesh: THREE.Mesh | THREE.Group | null;
  ...
  public spawn(
    ...
    (this.flash.material as THREE.MeshBasicMaterial).opacity = 1.0;
    this.flash.scale.set(1, 1, 1);
    (this.ring.material as THREE.MeshBasicMaterial).opacity = 1.0;
  ...
  public update(delta: number): void {
    ...
    (this.ring.material as THREE.MeshBasicMaterial).opacity = 1 - progress;
...
export class EliteWarpEffect {
  ...
  public screenShakeTriggered: boolean = false;
...
export class ExplosionEffect {
  ...
  public update(delta: number): void {
    ...
    for (let i = 0; i < this.particles.length; i++) {
      ...
      (trail.material as THREE.MeshBasicMaterial).opacity = Math.max(0, 0.6 - progress * 0.8);
    }
...
export class BossExplosionEffect {
  ...
  public shakeMagnitude: number = 0.5;

  /** The THREE.js scene this effect belongs to */
  private scene: THREE.Scene;
  /** Ring spawn timings (seconds) */
  private readonly ringTimings: number[] = [0, 0.15, 0.3];
  /** Whether each ring has been spawned */
  private ringSpawned: boolean[] = [false, false, false];
  ...
  public update(delta: number): void {
    ...
    for (let i = 0; i < this.rings.length; i++) {
      ...
      (ring.material as THREE.MeshBasicMaterial).opacity = 1 - ringProgress;
    }
  ...
  private spawnRing(position: { x: number; y: number; z: number }, index: number): void {
...
export class EffectManager {
  ...
  private warpEffects: WarpEffect[] = [];
  ...
  private bossExplosionEffects: BossExplosionEffect[] = [];

  /** Array of active elite warp effects */
  private eliteWarpEffects: EliteWarpEffect[] = [];

  /** Array of active elite explosion effects */
  private eliteExplosionEffects: EliteExplosionEffect[] = [];
   ...
   */
  public spawnHit(position: THREE.Vector3 | { x: number; y: number; z: number }): HitEffect {
   ...
   */
  public spawnExplosion(position: THREE.Vector3 | { x: number; y: number; z: number }): ExplosionEffect {
   ...
   */
  public spawnBossExplosion(position: THREE.Vector3 | { x: number; y: number; z: number }): BossExplosionEffect {
   ...
   */
  public getActiveBossExplosions(): BossExplosionEffect[] {
   ...
   */
  public spawnEliteExplosion(position: THREE.Vector3 | { x: number; y: number; z: number }): EliteExplosionEffect {
   ...
   */
  public getActiveEliteExplosions(): EliteExplosionEffect[] {
   ...
   */
  public getActiveEliteWarps(): EliteWarpEffect[] {
