 ...
 */
export const ELITE_ENEMY_TYPE_DATA = {
    reaper: {
        name: 'REAPER',
        color: '#2a2a2a',
...
export class Warden extends Enemy {
    ...
    constructor(scene, id) {
        ...
        Object.defineProperty(this, "laserCooldown", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: 5.0
     ...
     * @param playerPosition - The player's current position (for aiming)
     */
    fireLaser(bulletPool, playerPosition) {
        const pos = this.mesh.position;
    ...
    fire(bulletPool, playerPosition) {
        ...
        for (let i = 0; i < 3; i++) {
            ...
            if (missile) {
                ...
                missile.spawn({ x: pos.x + offsetX, y: pos.y - 0.4, z: pos.z }, playerPosition);
            }
        }
        this.fireCooldown = this.fireRate;
    }
...
export class Overlord extends Enemy {
    ...
    constructor(scene, id) {
        ...
        Object.defineProperty(this, "attackPhase", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: 0
     ...
     * @param playerPosition - The player's current position
     */
    fireAimedStream(bulletPool, pos, playerPosition) {
 ...
 */
export function createEliteEnemy(scene, id, type) {
    switch (type) {
        case 'reaper':
 ...
 */
export function getRandomEliteEnemyType() {
    const types = ['reaper', 'warden', 'harbinger', 'overlord'];
