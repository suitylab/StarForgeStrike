...
/**
 * EnemyBulletPool — a fixed-size object pool for managing reusable EnemyBullet instances.
 *
 * The pool pre-allocates a fixed number of EnemyBullet objects and recycles them via
 * get()/release() cycles. This avoids the performance cost of continuous mesh
 * creation/destruction, ensuring stable frame rates even with dense enemy bullet patterns.
 *
 * The pool is the sole source of enemy bullets in the game. All enemy bullet spawning
 * (Drone, Raider, Sentry, and future elite enemies) must request bullets through this pool.
 *
...
export class EnemyBulletPool {
  ...
  private pool: EnemyBullet[] = [];

  /** Set of currently active bullets for O(1) membership checks and removal */
  private activeSet: Set<EnemyBullet> = new Set();

  /** Array of active bullets for efficient iteration (kept in sync with activeSet) */
  private activeList: EnemyBullet[] = [];

  /** The THREE.js scene that bullets are added to */
  private scene: THREE.Scene;

  /**
   * Creates a new enemy bullet pool and pre-allocates the specified number of bullets.
   *
   * @param scene - The THREE.js scene to add bullet meshes to
   * @param initialSize - Number of bullets to pre-allocate (default: 100)
   */
  constructor(scene: THREE.Scene, initialSize: number = 100) {
    this.scene = scene;

    // Pre-allocate the pool with sequential ids (0..initialSize-1)
    for (let i = 0; i < initialSize; i++) {
      this.pool.push(new EnemyBullet(this.scene, i));
    }
  }
   ...
   *
   * @returns {EnemyBullet | null} An inactive bullet ready for spawning, or null if all bullets are active
   */
  public get(): EnemyBullet | null {
   ...
   */
  public release(bullet: EnemyBullet): void {
   ...
   *
   * @returns {EnemyBullet[]} Array of active bullets
   */
  public getActive(): EnemyBullet[] {
  ...
  /**
   * Removes every bullet mesh from the scene and disposes all
   * geometries and materials, freeing the GPU resources.
