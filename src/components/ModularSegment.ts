...
/**
 * LevelStyle — the visual theme for a corridor segment.
 * 'titan-gate': Level 1 cold steel military corridor with cyan conduits.
 * 'void-reactor': Level 2 organic-tech hybrid with purple membranes and energy cores.
 */
...
export class ModularSegment {
   ...
   */
  constructor(scene: THREE.Scene, yPosition: number, levelStyle: LevelStyle = 'titan-gate') {
    this.scene = scene;
   ...
   */
  public recycle(aheadY: number): void {
  ...
  private buildFloor(): void {
    ...
    // Floor sits at the top half of the segment so adjacent segments' floors and
    // ceilings butt together exactly — no overlapping slab wall at the joints,
    // leaving the centre of each corridor connection open for the player.
    floor.position.set(0, 0.5, -2);
    // Rotate 90° so the slab stands upright along the corridor instead of
    // lying flat across the flight path (which blocked the player).
  ...
  private buildWalls(): void {
    ...
    const leftWall = new THREE.Mesh(
      new THREE.BoxGeometry(0.5, 20, 8),
      wallMaterial
    );
  ...
  /**
   * Builds the left and right support pillars that frame the corridor.
   * Each segment contributes one full-height pillar per side (tiles seamlessly
   * with the neighbouring segment) so the junction is framed by two pillars —
   * one on each side — with the centre left completely open for the player.
   */
  private buildSupportBeams(): void {
    ...
    // Pillar positions on the left and right sides of the corridor
    const pillarXPositions = [-8, 8];
    
    for (const x of pillarXPositions) {
      // Full-height pillar running from the floor to the ceiling
      const pillar = new THREE.Mesh(
        new THREE.BoxGeometry(0.9, 20, 0.9),
        beamMaterial
      );
      pillar.position.set(x, 10, -2);
      this.mesh.add(pillar);
      
      // Accent strip on the inner edge of the pillar
      const innerX = x > 0 ? x - 0.45 : x + 0.45;
      const accent = new THREE.Mesh(
        new THREE.BoxGeometry(0.12, 19.2, 0.12),
        accentMaterial
      );
      accent.position.set(innerX, 10, -1.6);
  ...
  private buildConduits(): void {
    ...
    // Left wall conduits
    const leftConduit1 = new THREE.Mesh(
      new THREE.BoxGeometry(0.1, 18, 0.1),
      conduitMaterial
    );
    leftConduit1.position.set(-8.5, 10, -1);
    this.mesh.add(leftConduit1);
    
    const leftConduit2 = new THREE.Mesh(
      new THREE.BoxGeometry(0.1, 18, 0.1),
      conduitMaterial
    );
    ...
    // Right wall conduits
    const rightConduit1 = new THREE.Mesh(
      new THREE.BoxGeometry(0.1, 18, 0.1),
      conduitMaterial
    );
    ...
    const rightConduit2 = new THREE.Mesh(
      new THREE.BoxGeometry(0.1, 18, 0.1),
      conduitMaterial
    );
   ...
   */
  private buildOverheadLights(): void {
  ...
  private buildFloorGrates(): void {
    ...
    for (let i = 0; i < 5; i++) {
      const grate = new THREE.Mesh(
        new THREE.BoxGeometry(16, 0.02, 0.1),
        grateMaterial
      );
      grate.position.set(0, 0.72, -4 + i * 2);
      this.mesh.add(grate);
    }
  }
   ...
   */
  private buildPurpleMembranes(): void {
   ...
   */
  private buildBioluminescentGrowths(): void {
    ...
    const growthPositions = [
      // Floor growths
      { x: -6, y: 0.72, z: -4, type: 'sphere', color: 'green' },
      { x: -3, y: 0.72, z: -1, type: 'cone', color: 'purple' },
      { x: 2, y: 0.72, z: -3, type: 'sphere', color: 'purple' },
      { x: 5, y: 0.72, z: -5, type: 'cone', color: 'green' },
      { x: 7, y: 0.72, z: 0, type: 'sphere', color: 'green' },
    ...
    for (const pos of growthPositions) {
      ...
      let growth: THREE.Mesh;
      if (pos.type === 'sphere') {
        growth = new THREE.Mesh(
          new THREE.SphereGeometry(0.15, 8, 8),
          material
        );
      } else {
   ...
   */
  private buildEmergencyLights(): void {
   ...
   */
  private buildPurpleAmbientLight(): void {
   ...
   */
  private buildSovereignCoreDecorations(): void {
   ...
   */
  private buildWhiteFloorCeiling(): void {
    // Royal navy floor slab
    const floorMaterial = new THREE.MeshStandardMaterial({
      color: 0x223356,
      metalness: 0.4,
      roughness: 0.5,
    ...
    // Navy panel lines on the floor (subtle grid)
    ...
    for (let i = 0; i < 5; i++) {
      const line = new THREE.Mesh(
        new THREE.BoxGeometry(16, 0.02, 0.08),
        panelLineMaterial
      );
      line.position.set(0, 0.72, -4 + i * 2);
      this.mesh.add(line);
    }
  ...
  private buildGoldTrim(): void {
    ...
    const floorLeftTrim = new THREE.Mesh(
      new THREE.BoxGeometry(0.15, 0.1, 8),
      goldTrimMaterial
    );
    floorLeftTrim.position.set(-7.9, 0.72, -2);
    this.mesh.add(floorLeftTrim);

    const floorRightTrim = new THREE.Mesh(
      new THREE.BoxGeometry(0.15, 0.1, 8),
      goldTrimMaterial
    );
    floorRightTrim.position.set(7.9, 0.72, -2);
    this.mesh.add(floorRightTrim);

    // Ceiling edge trims (left and right edges)
    const ceilingLeftTrim = new THREE.Mesh(
      new THREE.BoxGeometry(0.15, 0.1, 8),
      goldTrimMaterial
    );
    ...
    const ceilingRightTrim = new THREE.Mesh(
      new THREE.BoxGeometry(0.15, 0.1, 8),
      goldTrimMaterial
    );
    ...
    const leftWallBottomTrim = new THREE.Mesh(
      new THREE.BoxGeometry(0.1, 0.15, 8),
      goldTrimMaterial
    );
    leftWallBottomTrim.position.set(-8.2, 0.72, -2);
    this.mesh.add(leftWallBottomTrim);

    // Left wall top edge
    const leftWallTopTrim = new THREE.Mesh(
      new THREE.BoxGeometry(0.1, 0.15, 8),
      goldTrimMaterial
    );
    ...
    const rightWallBottomTrim = new THREE.Mesh(
      new THREE.BoxGeometry(0.1, 0.15, 8),
      goldTrimMaterial
    );
    rightWallBottomTrim.position.set(8.2, 0.72, -2);
    this.mesh.add(rightWallBottomTrim);

    // Right wall top edge
    const rightWallTopTrim = new THREE.Mesh(
      new THREE.BoxGeometry(0.1, 0.15, 8),
      goldTrimMaterial
    );
   ...
   */
  private buildHolographicDisplays(): void {
    ...
    for (const config of displayConfigs) {
      ...
      const display = new THREE.Mesh(
        new THREE.PlaneGeometry(0.3, 3.5),
        material
      );
      ...
      const frame = new THREE.Mesh(
        new THREE.BoxGeometry(0.05, 3.7, 0.05),
        frameMaterial
      );
   ...
   */
  private buildEnergyPillars(): void {
    // Outer pillar material (royal navy)
    ...
    for (const pos of pillarPositions) {
      // Outer pillar
      const pillar = new THREE.Mesh(
        new THREE.CylinderGeometry(0.4, 0.45, 14, 12),
        pillarMaterial
      );
   ...
   */
  private buildGoldAmbientLight(): void {
   ...
   */
  private animateHolographicDisplays(): void {
   ...
   */
  private animateEnergyPillars(): void {
   ...
   */
  private animateGoldTrimLights(): void {
   ...
   */
  private animateWarningLights(): void {
    const pulse = Math.sin(this.pulseTime * 3) * 0.5 + 0.5; // 0 to 1
   ...
   */
  private animateEnergyCores(): void {
  ...
  private animateGrowths(): void {
    ...
    for (const growth of this.growths) {
      ...
      const material = growth.material as THREE.MeshBasicMaterial;
      material.opacity = 0.6 + pulse * 0.4;
    }
  }
   ...
   * @param delta - Time elapsed since last frame in seconds
   */
  private animateEmergencyLights(delta: number): void {
...
export class ModularCorridor {
  ...
  private segments: ModularSegment[] = [];
   ...
   */
  constructor(scene: THREE.Scene, segmentCount: number = 8, levelStyle: LevelStyle = 'titan-gate') {
    this.scene = scene;
    this.levelStyle = levelStyle;
   ...
   */
  public getLevelStyle(): LevelStyle {
