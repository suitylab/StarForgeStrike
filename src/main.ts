...
import { PlaneType } from './components/GameConfig';
import './style.css';
...
// Hide the player HUD while on the main menu — it is only shown during gameplay.
...
const uiManager = new UIManager(
  ...
  camera,
  (planeType: PlaneType) => {
        // Show the player HUD now that gameplay is starting
        hud.setVisible(true);
    ...
    // The onVictory callback shows the victory screen when all levels are complete.
        game = new Game(scene, camera, hud, planeType, (level: number, breakdown: { enemies: number; elites: number; boss: number; pickups: number; time: number; health: number; total: number }) => {
  ...
  () => {
    ...
    // Fully dispose the game so gameplay objects (corridor, effects) are
    // removed from the scene before showing the menu.
    if (game) {
      game.dispose();
      game = null;
    }
    // Hide the player HUD when returning to the main menu
    hud.setVisible(false);
    uiManager.showMenu();
  },
  () => {
  ...
  () => {
    ...
    // Fully dispose the game so gameplay objects are removed from the scene.
    if (game) {
      game.dispose();
      game = null;
    }
